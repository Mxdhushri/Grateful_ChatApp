...
const MessageBar = () => {
  const [message, setMessage] = useState("") //initially blank, then input message set
  const emojiRef = useRef()
  const[emojiPickerOpen, setEmojiPickerOpen] = useState(false)
  
  useEffect(() => {
    function handleClickOutside(event) {
      // code to close emoji pallet if user clicks ouside pallet boundary
  ...
  const handleSendMessage = async () => { //first define this then call below
  ...
  return (
    <div className="h-[10vh] bg-[#1c1d25] flex justify-center items-center px-8 mb-6 gap-6">
      <div className="flex-1 flex bg-[#2a2b33] rounded-md items-center gap-5 pr-5">
        ...
        <button button className="text-neutral-500 focus:border-none focus:outline-none focus:text-white duration-300 transition-all">
